...
| 🟢 초록 원 | 내 위치 | - |
| 🦁🤖✈️ (금색) | 내 수호신 | - |
| 🦁🤖✈️ (빨간 테두리) | 다른 플레이어 | 전투/동맹 선택 |
| 🛡️ (파란 테두리) | 방어형 고정 수호신 | 직접 공격 |
| ⚙️ (금색 테두리) | 생산형 고정 수호신 | 직접 공격 |
...
#### 겹침 방지
- 인접한 마커는 그리드 패턴으로 자동 분산
- 약 0.0002도(~15m) 간격 오프셋 적용
- divIcon 사용으로 마커 렌더링 최적화

### 7. 알림 시스템

#### 탐지 알림
- 주변 플레이어/고정 수호신 발견 시 "탐지" 버튼 활성화
- 버튼 클릭으로 탐지 패널 열기 (자동 팝업 X)
- 배지로 탐지된 개체 수 표시

#### 영역 침범 알림
- 다른 플레이어 영역 진입 시 1회만 알림
- 동일 영역 내에서는 중복 알림 없음
- 영역 벗어나면 초기화 (재진입 시 다시 알림)
...
### Phase 1.5 (2024-04-20 완료)
- [x] PWA 지원 (앱 설치 기능)
- [x] 푸시 알림 기반 구축 (Service Worker)
- [x] 탐지 시스템 개선 (버튼 클릭 방식으로 변경)
- [x] 영역 침범 알람 1회만 표시
- [x] 직접 전투/협력 (플레이어 마커 클릭)
- [x] 고정 수호신 직접 공격 (마커 클릭)
- [x] 마커 겹침 방지 (그리드 패턴 분산)
...
### 앱 설치
- 모바일/데스크톱에서 홈 화면에 앱 추가 가능
- manifest.json으로 앱 아이콘, 테마 색상 설정
- standalone 모드로 네이티브 앱처럼 실행

### Service Worker
- 네트워크 우선 전략 (항상 최신 버전 로드)
- 오프라인 폴백 캐싱
- 푸시 알림 수신 준비 완료
