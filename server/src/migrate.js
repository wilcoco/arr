...
async function migrate() {
  ...
  try {
    ...
    // R) 베타 풍부 boost v3 — 새 마커로 1회 더 실행 (이전 v1/v2는 5000E/Lv3까지만)
    //   v3: 모든 사용자 energy ≥12000, level ≥10, guardian 스탯 ~10배 (cap 안 넘게)
    const boostV3Done = await db.query(`SELECT name FROM _migrations WHERE name = 'beta_full_boost_v3_2026_05_07'`)
    if (boostV3Done.rows.length === 0) {
      const u = await db.query(
        `UPDATE users
         SET energy_currency = GREATEST(energy_currency, 12000),
      ...
      await db.query(`INSERT INTO _migrations (name) VALUES ('beta_full_boost_v3_2026_05_07')`)
      console.log(`[migrate] beta_full_boost_v3: ${u.rowCount} users + ${g.rowCount} guardians boosted (energy≥12000, lv≥10)`)
    }
...
async function ensureSchema() {
  ...
  await db.query(`
    CREATE TABLE IF NOT EXISTS users (
      ...
      energy_currency INT DEFAULT 12000,
      level INT DEFAULT 1,
      xp INT DEFAULT 0,
